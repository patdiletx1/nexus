...
- Frontend Flutter: bootstrap local activo en `frontend/` con estructura por capas (`pages/services/widgets/models`), flujo `health/profile/sync/list/warmup/score`, bloque Ops (`/v1/ops/alerts` con panel visual priorizado + resumen `/metrics`), persistencia local y controles de sesion (`mostrar/ocultar token`, `limpiar sesion local` con confirmacion y feedback).
